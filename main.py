...
def get_athlete_welness(
    intervals_api_key, base_url, athlete_id, wellness_startdate, wellness_enddate
):
    url = "{}/api/v1/athlete/{}/wellness{{ext}}?oldest={}&newest={}".format(
        base_url, athlete_id, wellness_startdate, wellness_enddate
    )
...
def get_athlete_activities(
    intervals_api_key, base_url, athlete_id, activities_startdate, activities_enddate
):
    url = "{}/api/v1/athlete/{}/activities?oldest={}&newest={}".format(
        base_url, athlete_id, activities_startdate, activities_enddate
    )
...
athlete_info = get_athlete_info(
    intervals_api_key=intervals_api_key, base_url=base_url, athlete_id=athlete_id
).json()
...
if len(athlete_info["sportSettings"]) == 0:
    ...
    for sport_setting in athlete_info["sportSettings"]:
        print(
            sport_setting["id"],
            sport_setting["ftp"],
            sport_setting["created"],
            sport_setting["updated"],
            sport_setting["max_hr"],
        )
...
if len(wellness_info) == 0:
    ...
    for wellness_data in wellness_info:
        print(
            wellness_data["id"],
            wellness_data["updated"],
            wellness_data["ctl"],
            wellness_data["atl"],
            wellness_data["restingHR"],
            wellness_data["sportInfo"],
        )
