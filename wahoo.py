...
def main():
    ...
    access_token = refresh(json.load(open(wahoo_cfg))) if os.path.isfile(wahoo_cfg) else authenticate()

    wahoo_user_info = get_wahoo_user( access_token)
    print( wahoo_user_info)
    print( 'Set weight to 92')
    weight = '92'


    # wahoo_user_set_weight = set_wahoo_user_weight( access_token, weight)
